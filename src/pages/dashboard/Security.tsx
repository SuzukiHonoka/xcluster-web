...
import {
  ...
  fDisable,
  fEnable,
} from "../../features/acls/aclsSlice.ts";
...
const PluginManage = () => {
  ...
  const aclsError = useAppSelector(selectError);
  const aclsStatus = useAppSelector(selectStatus);
  const rows = useAppSelector(selectAcls);

  // show success or error status
  useEffect(() => {
    console.log("aclsStatus:", aclsStatus);
    if (aclsStatus === "succeed") {
      alert("Operation succeed", "success");
    } else if (aclsStatus === "failed") {
      alert(aclsError!, "error");
    }
    return () => {
      dispatch(resetStatus());
    };
  }, [alert, dispatch, aclsError, aclsStatus]);
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      valueGetter: ({ row }: GridValueGetterParams) =>
        row.mode === 1 ? "whitelist" : "blacklist",
    },
    {
      field: "srcType",
      type: "string",
